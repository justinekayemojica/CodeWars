...
public class ConsecutiveStrings {
	...
	public static String longestConsec(String[] strarr, int k) {
		...
		for(int i = 0 ; i <= strarr.length - k ; i++) {
			String longestStr = "";
			for (int c = 0 ; c < k ; c ++) {
				longestStr = longestStr.concat(strarr[i+c]);
			}
			conStrList.add(longestStr.trim());
		}
		return conStrList.isEmpty() ? "" : conStrList.stream().max(Comparator.comparingInt(String::length)).get();
    }
